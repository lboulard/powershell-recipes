...
SET ARCHIVE=
FOR /D %%d IN ("initool-*") DO (
  FOR %%f IN ("%%d\initool-v*-win32.zip") DO (
...
IF %VERSION%==notfound (
 ECHO ** ERROR: No initool archive found
 CALL :errorlevel 64
 GOTO :exit
)
...
SET "WORKDIR=%TEMP%\initool-%VERSION%"
SET "OUTDIR=%TEMP%\initool-%VERSION%"
SET "BINEXE=%OUTDIR%\initool.exe"
SET "BINDLL=%OUTDIR%\camlrt.dll"

SET "DEST=%LBPROGRAMS%\bin"
SET "BIN=%DEST%\initool-%VERSION%.exe"
SET "DLL=%DEST%\camlrt-%VERSION%.dll"
SET "BINLINK=%DEST%\initool.exe"
SET "DLLLINK=%DEST%\camlrt.dll"
...
@IF EXIST "%DLLLINK%" DEL /Q /F "%DLLLINK%"
@IF ERRORLEVEL 1 GOTO :exit

MKLINK /H "%BINLINK%" "%BIN%"
MKLINK /H "%DLLLINK%" "%DLL%"
