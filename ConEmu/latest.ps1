...
$project = "Maximus5/ConEmu"
$tagPattern = "/(?<tag>v(?<version>\d+\.\d+(\.\d+){0,2}))$"
$wanted = "^ConEmu(Pack|Setup)\.\d+\.(7z|exe)$"
...
$files = Find-GitHubAssets $project $tag $version -AssetPattern $wanted -Token $githubToken
...
if ($files) {
  Get-Url $files -Headers $headers -ProjectName conemu
}
